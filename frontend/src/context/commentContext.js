...
export const CommentContextProvider = (props) => {
  ...
  const createUser = async (user, slug) => {
    try {
      ...
      const { data } = await axios.post(
        `/api/v1/viewer/create/${slug}`,
        { full_name: user.name, otp: user.otp },
        config
      );
  ...
  const logout = async () => {
    axios.delete("/api/v1/viewer/logout");
  ...
  const getAllCommentsForAdmin = async () => {
    try {
      ...
      dispatchUser({
        type: "GET_ALL_COMMENTS_FOR_ADMIN_SUCCESS",
        payload: data.responses,
      });
    } catch (error) {
      dispatchUser({
        type: "GET_ALL_COMMENTS_FOR_ADMIN_FAIL",
        payload: error?.response?.data.message,
      });
      dispatchUser({ type: "CLEAR_ERRORS" });
    }
  ...
  const updateComment = async (comment, status) => {
    try {
      const { data } = await axios.put(
        `/api/v1/responses/${comment._id}`,
        { ...comment, status: status },
        config
      );
      const index = commentState.allComments.findIndex(
        (response) => response._id === comment._id
      );
